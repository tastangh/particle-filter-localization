...
public:
    ...
    void motionUpdate(const nav_msgs::Odometry& odom_msg) {
        // Simple motion model placeholder
        for (auto& p : particles_) {
            p.x += 0.01 * cos(p.theta);
            p.y += 0.01 * sin(p.theta);
            // theta update omitted for simplicity
        }
    }

    void sensorUpdate(const sensor_msgs::LaserScan& scan_msg) {
        // Weight update placeholder
        for (auto& p : particles_) {
            p.weight = 1.0; // simplistic uniform weight
        }
    }
...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& msg) {
    if (pf_ptr) {
        ...
        pose_msg.pose.orientation.w = cos(est.theta / 2);
        pose_msg.pose.orientation.z = sin(est.theta / 2);
